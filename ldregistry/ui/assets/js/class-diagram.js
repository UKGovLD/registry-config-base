...
//            type: 'devs.Model',   // To match up to devs.ModelView
    ...
    var indexClass = function(cells, classIndex, spec) {
        ...
        var properties = Object.keys(spec.properties)

        var cls = new OwlClass({
            size: {
                width: WIDTH,
                height: Math.max(properties.length * 20 + 20, HEIGHT)
            },
            name: wraptext,
            attributes: properties.map( k => k + ( spec.properties[k].type ? ' (' + spec.properties[k].type + ')' : '') )
        });
